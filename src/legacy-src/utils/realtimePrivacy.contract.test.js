...
const groupAddPrivacy = readLegacy('utils/groupAddPrivacy.js');
...
assert(message.includes('resolveGroupAddPrivacy'));
assert(groupAddPrivacy.includes('follower: { $in: followingRestrictedIds }'));
assert(groupAddPrivacy.includes('following: actorId'));
